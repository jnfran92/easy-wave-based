...
k_x = 5    # Space domain discrete freq
...
for time_step in range(2, int(num_div_t)):
    ...
    for n_p in range(n_partitions):
        ...
        # force_k[n_p] = dct(dct(f_n_field[n_p].reshape([num_div_y, num_div_x])).T).T
    ...
    for y_ih in range(int(num_div_y/2)-5, int(num_div_y/2)):
